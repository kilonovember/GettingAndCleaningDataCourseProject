...
|-----------------|     |-----------------|----------------------------------------------|---------------------|
|trainTable       |13               |a table to hold the data from X_train.txt        ||
|testTable        |17               |a table to hold the data from X_test.txt         ||
|CombinedDataSets |21               |a table to hold combined trainTable and testTable||
|colHeads         |25               |a table to hold the data from features.txt       ||
|SelectedData     |33               |a subset of CombinedDataSets holding only columns with mean and standard deviation data||
|trainActivity    |37               |a vector to hold training activity identifiers   ||
|testActivity     |41               |a vector to hold testing activity identifiers    ||
|activity         |45               |a vector to hold the merged trainActivity and testActivity identifers||
|activityLabels   |50               |a table matching activity identifiers with their text descriptions||
|activityTextValues|51              |a vector containing only the text description column data from activityLabels||
|ActivitiesSelectedData|61          |a table comprised of the activity text values bound to the SelectedData table||
|trainingSubjIDs|66                 |a vector to hold the training subject identifiers||
|testSubjIDs    |70                 |a vector to hold the testing subject identifiers||
|subject        |73                 |a vector to hold the merged training and testing subject identifiers||
|Step.5.1.SelectedData|77           |a table comprised of the subject identifiers bound to the ActivitiesSelectedData table||
|
